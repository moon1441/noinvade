...
public interface DeviceAuthMapper {
    ...
    @Select("SELECT * FROM device_auth where account=#{familyID} and device_type=#{type}")
    @Results({
            @Result(property = "type", column = "device_type"),
            @Result(property = "status", column = "device_auth_status"),
            @Result(property = "familyID", column = "account")}
    ...
    DeviceAuth getAuthByFamilyIDAndType(String familyID,int type);
    ...
    @Insert("INSERT INTO device_auth_record(account,device_auth_status,device_type,auth_time)"+
    ...
    @Update("Update device_auth set device_auth_status=#{status},update_time=now()"+
            "where account=#{familyID} and device_type=#{type}")
    int update(DeviceAuth deviceAuth);

    @Insert("INSERT INTO device_auth(account,device_auth_status,device_type)"+
