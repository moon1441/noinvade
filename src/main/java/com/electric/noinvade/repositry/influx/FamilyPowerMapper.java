...
public interface FamilyPowerMapper {
    ...
    //当天实时总功率（曲线）
    @InfluxQuery("select * from raw_overall_10s where time > ?ms and meter_id=? and phase=? order by time asc")
    List<FamilyPower> getFamilyDailyPower(long startTime,String meterID,String phase);

    //单用户累计电量
    @InfluxQuery("select sum(ep) from ep_1h where time > ?ms and meter_id=? and phase=?")
    List<FamilyEPower> getFamilyAllEPower(long startTime,String meterID,String phase);

    //所有用户区间累计电量
    @InfluxQuery("select sum(ep) from ep_1h where time > ?ms and time <=?ms and meter_id=? and phase=?")
