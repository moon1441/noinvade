...
public interface FamilyPowerMapper {
    ...
    //当天实时总功率（曲线）
    @InfluxQuery("select time,meter_id,p? as power from raw_aggr_10s where time > ?ms and meter_id='?' order by time asc")
    List<FamilyPower> getFamilyDailyPower(String phase,long startTime,String meterID);

    //单用户累计电量
    @InfluxQuery("select time,sum(ep?) as power from ep_1h where time>?ms and meter_id='?'")
    List<FamilyEPower> getFamilyAllEPower(String phase,long startTime,String meterID);

    //所有用户区间累计电量
    @InfluxQuery("select time,sum(ep?) as power from ep_1h where time > ?ms and time <=?ms and meter_id='?'")
    ...
    List<FamilyDeviceEPower> getSumFamilyDeviceEPower(long start, long end,String meterID, String types);


    //户内设备区间段汇总功率信息
    @InfluxQuery("select time,meter_id,p from device_aggr_10s where time>=?ms and time<?ms and meter_id='?' and  type='?' and phase='?' ")
    List<FamilyDevicePower> getFamilyDevicePower(long start, long end,String meterID,String type,String phase);
